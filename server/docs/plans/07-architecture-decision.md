...
# ADR-8. 스키마 변경을 어떻게 관리할 것인가

운영은 처음부터 `ddl-auto: validate`였다. 즉 **스키마를 만든 DDL이 어디에도 없다.**
누군가 손으로 실행했고 그 SQL은 기록되지 않았다.
반면 테스트는 `ddl-auto: create`라 엔티티에서 스키마를 새로 만들었다.
그래서 **운영에서 실제로 도는 DDL은 한 번도 검증된 적이 없다.**

랭킹 인덱스를 추가하려 했을 때 이게 드러났다. 운영에 반영할 경로가 없었다.
...
| 장점 | 단점 |
| --- | --- |
| 도구 추가 없음 | 어느 환경이 어디까지 적용됐는지 알 수 없음 |
| DDL을 사람이 완전히 통제 | 적용을 사람이 기억해야 함. 롤링 배포와 순서를 맞추기 어려움 |
| | 스크립트가 리포지터리 밖에 있어 리뷰도 이력도 없음 |

### B. Flyway

| 장점 | 단점 |
| --- | --- |
| 적용 이력이 DB에 남음 (`flyway_schema_history`) | 적용된 스크립트는 수정 불가 (체크섬) |
| SQL 그대로 씀. 배울 것이 거의 없음 | 롤백 스크립트를 자동으로 만들어 주지 않음 |
| 구동 시 자동 적용이라 롤링 배포와 맞음 | |

### C. Liquibase

| 장점 | 단점 |
| --- | --- |
| 롤백을 기술할 수 있음 | changelog(XML/YAML) 문법을 따로 익혀야 함 |
| DB 중립 | MySQL만 쓸 것이라 중립성이 이득이 아님 |

## 결정: **B. Flyway**

구조가 확정된 지금을 `V1__init.sql`로 잡는다.
**테스트도 `ddl-auto: validate` + Flyway로 돌린다.**

## 근거

1. 인덱스 하나를 반영할 방법이 없는 상태였다. 배포 전에 풀어야 할 매듭이었다
2. 테스트가 `create`로 따로 스키마를 만들면 마이그레이션이 틀려도 아무도 모른다.
   같은 스크립트를 쓰게 하니 엔티티와 DDL 불일치가 곧바로 빌드 실패가 된다
3. 쓸 DB가 MySQL 하나뿐이라 Liquibase의 중립성이 이득이 아니다.
   SQL을 그대로 쓰는 쪽이 읽기도 쉽다
4. 롤백은 어차피 앞으로 굴러가는 스크립트로 처리한다. 운영 중 자동 롤백은 쓰지 않을 것이다

---

# ADR-9. 외래키를 DB에 걸 것인가
...
| 장점 | 단점 |
| --- | --- |
| 참조 무결성을 DB가 보장 | 쓰기마다 부모 행에 잠금이 걸림 |
| | 삭제·아카이빙·파티셔닝 때 제약이 걸림 |
| | 테스트 정리 순서가 강제됨 |
...
| 장점 | 단점 |
| --- | --- |
| 잠금 경합 없음 | 잘못된 참조가 들어갈 수 있음 |
| 데이터 이관·정리가 자유로움 | 고아 행을 스스로 찾아야 함 |

## 결정: **B. 외래키를 걸지 않는다**

`Match`의 `@ManyToOne`에 `@ForeignKey(NO_CONSTRAINT)`를 명시한다.
누가 `ddl-auto`를 켜도 제약이 생기지 않게 하기 위함이다.

## 근거

1. `matches`는 원장이라 추가만 되고 수정·삭제가 없다. 무결성이 깨질 경로가 좁다
2. 경기 결과 기록이 사용자 행에 잠금을 잡을 이유가 없다
3. 인덱스는 조회가 실제로 쓰는 것만 남긴다. 외래키가 자동으로 만들던
   `winner_id` 인덱스는 어떤 쿼리도 쓰지 않아 제거했다

---

# ADR-10. 시각을 어느 타임존으로 저장할 것인가

기존에는 `hibernate.jdbc.time_zone`, JDBC URL의 `serverTimezone`, JVM의 `user.timezone`이
모두 `Asia/Seoul`이었고, **테스트에만 이 설정이 빠져 있었다.**
반복 대전 감쇠가 24시간 윈도를 쓰는데, 그 쿼리가 운영과 다른 설정에서 검증되고 있었다.
...
| 장점 | 단점 |
| --- | --- |
| DB를 직접 열어 볼 때 읽기 편함 | 서버 지역이 바뀌면 과거 값 해석이 불가능 |
| | 서머타임이 있는 지역을 다루게 되면 중복·누락 시각이 생김 |

### B. UTC 저장 + 표시할 때 변환

| 장점 | 단점 |
| --- | --- |
| 값이 한 가지로만 해석됨 | DB를 직접 볼 때 9시간을 암산해야 함 |
| 지역·서머타임과 무관 | |

## 결정: **B. UTC 저장**

`hibernate.jdbc.time_zone: UTC`, JDBC는 `connectionTimeZone=UTC`.
**JVM 기본 타임존은 `Asia/Seoul`로 둔다** — 로그를 읽는 사람이 한국에 있기 때문이다.
테스트도 운영과 같은 설정을 쓴다.

## 근거

1. `Match.finishedAt`은 `Instant`다. 애초에 시점을 나타내는 타입이라 UTC 저장이 자연스럽다
2. 저장과 표시를 분리하면 나중에 지역을 늘려도 데이터는 그대로 쓸 수 있다
3. `serverTimezone`은 Connector/J의 구버전 별칭이다. `connectionTimeZone`으로 바꿨다

---

# ADR-11. 토큰을 어떻게 전달하고 보관할 것인가

[ADR-5](#adr-5-토큰)에서 `Authorization` 헤더로 가기로 정했지만 구현은 계속 쿠키였다.
그 사이에 드러난 것이 두 가지 있다.

- 프론트가 GitHub Pages(`ldhbenecia.github.io`)라 서버와 **cross-site**다.
  쿠키를 쓰려면 `SameSite=None; Secure`여야 하고, 그러면 브라우저가 알아서 붙여 주므로
  CSRF 방어를 따로 해야 한다. 지금 `csrf`는 꺼져 있다
- STOMP 핸드셰이크는 브라우저가 보내는 평범한 HTTP 요청이라 **헤더를 붙일 수 없다.**
  즉 헤더로 바꾸면 소켓 인증 시점도 옮겨야 한다

## 로그인 성공 후 토큰을 어떻게 넘길 것인가

### A. 쿠키 (현행)

| 장점 | 단점 |
| --- | --- |
| `HttpOnly`로 JS에서 못 읽음 | cross-site라 `SameSite=None; Secure` 필요 → CSRF 대책 필수 |
| 프론트가 할 일이 없음 | STOMP 핸드셰이크 말고는 쓸 데가 없어짐 |

### B. 쿼리스트링 리다이렉트

| 장점 | 단점 |
| --- | --- |
| 구현이 가장 쉬움 | 액세스 로그·`Referer`·방문 기록에 토큰이 남음 |

### C. 프래그먼트 리다이렉트

| 장점 | 단점 |
| --- | --- |
| 프래그먼트는 서버로 전송되지 않아 로그에 안 남음 | 프론트가 읽어서 저장하고 주소창을 지워야 함 |

### D. 일회용 코드 + 교환 엔드포인트

| 장점 | 단점 |
| --- | --- |
| 토큰이 URL에 아예 안 실림 | 코드 저장소와 만료 관리가 늘어남 |

## 결정: **C. 프래그먼트**

`${DOMAIN}/auth/callback#token=...`로 보내고, 프론트가 읽은 뒤
`history.replaceState`로 주소창에서 지운다.

## 보관 위치: **`localStorage`**

| 후보 | 새로고침 | XSS | 판단 |
| --- | --- | --- | --- |
| 메모리 | 잃음 | 안전 | 리프레시 토큰이 없어 매번 재로그인. 기각 |
| `localStorage` | 유지 | 읽힘 | 채택 |

## STOMP: **CONNECT 프레임 헤더**

핸드셰이크에 헤더를 붙일 수 없으므로 `CONNECT` 프레임의 `Authorization` 헤더로 받고,
`ClientFrameGuard`가 거기서 주체를 확정한다. 실패하면 연결을 거절한다.

## 근거

1. cross-site 쿠키는 CSRF 대책을 새로 만들어야 한다. 헤더는 브라우저가 자동으로
   붙이지 않으므로 CSRF가 애초에 성립하지 않는다
2. 프래그먼트는 서버로 가지 않는다. 쿼리스트링과 달리 액세스 로그·`Referer`에 남지 않는다
3. 일회용 코드가 더 안전하지만, 리프레시 토큰도 무효화 목록도 없는 지금 단계에서
   그 하나만 강화하는 것은 균형이 맞지 않는다

## 남은 것

`localStorage`는 XSS에 노출된다. 유효기간을 짧게 두는 것으로 감수하고 있으며,
리프레시 토큰과 무효화 목록은 [ADR-6](#adr-6-redis)의 Redis 도입 때 함께 다룬다.

---

# ADR-12. 비로그인 사용자를 코드에서 무엇이라 부를 것인가

[08](08-guest-mode-design.md)은 비로그인 사용자를 `GUEST` / `ROLE_GUEST`로 적었다.
그런데 [용어집](../../.claude/rules/java-conventions.md)은 `guest`를 **방의 참가자**(두 번째 플레이어)로
이미 못박아 두었고 코드도 그렇게 쓰고 있다 — `Room.guestId`, `RoomDetailResponse.guestId`,
`FirstMoveRule.GUEST_FIRST`.

둘을 겹쳐 쓰면 `RoomService` 한 파일 안에 `room.guestId()`(참가자)와
`user.isGuest()`(비로그인)가 나란히 나온다. 읽는 사람이 구분할 방법이 없다.

## 선택지

### A. 08 문서대로 `GUEST`를 겹쳐 쓴다

| 장점 | 단점 |
| --- | --- |
| 설계 문서와 식별자가 1:1 | 한 낱말이 두 뜻. 용어집이 무너진다 |
| 화면 문구("게스트")와 같음 | 방의 참가자가 비로그인일 때 `guest.isGuest()` 같은 문장이 나옴 |

### B. 비로그인 쪽을 `ANONYMOUS`로

| 장점 | 단점 |
| --- | --- |
| 뜻이 정확 | Spring Security의 `anonymousUser` / `ROLE_ANONYMOUS`와 정면 충돌 |
| | "인증 안 됨"과 "인증된 비로그인 신원"이 같은 낱말이 됨. 더 나쁨 |

### C. 비로그인 쪽을 `VISITOR`로

| 장점 | 단점 |
| --- | --- |
| 용어집의 `guest`와도, Spring의 `anonymous`와도 겹치지 않음 | 08 문서와 식별자가 달라짐 |
| 화면 문구는 "게스트"로 그대로 둘 수 있음 | 용어집에 한 줄 추가해야 함 |

### D. 방의 참가자를 `challenger` 등으로 개명

| 장점 | 단점 |
| --- | --- |
| 의미가 가장 정확해짐 | `Room`·DTO·프론트 `types.ts`까지 번진다 |
| | 이미 맞게 도는 것을 고치는 일. 이득 대비 비용이 큼 |

## 결정: **C. `VISITOR`**

`Provider.VISITOR` / `ROLE_VISITOR` / `User.isVisitor()` / `POST /api/v1/auth/visitors`.
**사용자에게 보이는 문구는 "게스트"를 그대로 쓴다.**

## 근거

1. 용어집은 "같은 개념을 코드마다 다르게 부르던 것"을 끝내려고 만든 표다.
   그 표를 세우자마자 한 낱말에 두 뜻을 얹으면 표가 있으나 마나 해진다
2. `ANONYMOUS`는 프레임워크가 선점한 낱말이다. 게다가 이 신원은 익명이 아니라
   **서버가 발급한 신원**이라 뜻도 틀리다
3. 코드 식별자와 화면 문구가 다른 것은 이 프로젝트에 이미 있는 방식이다
   (`mill` ↔ "3연속"). 새로 만드는 예외가 아니다
4. 참가자 쪽을 고치는 편이 의미는 더 정확하지만, 잘 도는 코드를 프론트까지 건드려야 한다

---

# ADR-13. 비로그인 신원 발급의 남용을 무엇으로 막을 것인가

`POST /api/v1/auth/visitors`는 **인증 없이 부를 수 있는 쓰기 엔드포인트**다.
막지 않으면 `users`에 행이 무한히 생긴다.

[ADR-6](#adr-6-redis를-어디에-쓸-것인가)은 레이트 리밋을 Redis 용도 ④로 적어 두었지만
Redis는 아직 도입되지 않았다.
...
| 장점 | 단점 |
| --- | --- |
| ADR-6과 그대로 맞음 | 이 기능 하나 때문에 의존성·compose·설정이 함께 는다 |
| 서버가 여러 대여도 맞음 | 서버는 아직 한 대다. 지금은 이득이 없다 |

### B. 인메모리 카운터

| 장점 | 단점 |
| --- | --- |
| 의존성 없음. 한 대에서는 정확 | 서버가 늘면 한도가 대수만큼 늘어남 |
| 재시작하면 카운터가 날아감 — 이 용도에선 손해가 아님 | 만료 항목을 스스로 치워야 함 |

### C. CAPTCHA

| 장점 | 단점 |
| --- | --- |
| 자동화에 강함 | "로그인이 장벽"이라며 없앴는데 다른 장벽을 세우는 일 |
| | 외부 서비스 의존 |

## 결정: **B. 인메모리 카운터** (IP당 시간당 10회)

Redis 전환에 대비해 한도 판단을 `VisitorIssueLimiter` 한 곳에만 둔다.
서버가 여러 대가 되는 시점에 이 클래스의 구현만 바꾼다.

## 근거

1. [ADR-3](#adr-3-데이터베이스)에서 당분간 **단일 인스턴스**로 못박았다.
   한 대에서 인메모리 카운터는 Redis와 정확도가 같다
2. 재시작하면 카운터가 초기화되지만, 공격자가 그걸 노리려면 서버를 재시작시켜야 한다.
   그럴 수 있으면 레이트 리밋이 문제가 아니다
3. CAPTCHA는 이 규모에 과하고, 진입 장벽을 없애려는 이 기능의 목적과 어긋난다
4. 남는 행은 정리 배치(보존 30일)가 함께 막는다. 리밋과 정리는 서로 보완한다

## 전제

`server.forward-headers-strategy: framework`를 켜서 `getRemoteAddr()`이
프록시가 아니라 실제 호출자를 돌려주게 했다. **이 헤더는 위조할 수 있으므로
앱이 프록시를 거치지 않고 직접 닿으면 한도를 우회당한다.**
[ADR-4](#adr-4-배포-토폴로지와-https)의 "오리진은 보안그룹으로 CloudFront만 허용"이
지켜지는 동안에만 맞는 설정이다.

---

# ADR-14. `users.nickname`에 유니크 제약을 걸 것인가

[08](08-guest-mode-design.md) 2절은 "닉네임 자동 생성 시 충돌을 DB가 막아준다"며
유니크 제약을 걸라고 적었다. 그런데 현재 카카오 로그인은 **카카오 닉네임을 그대로 저장한다.**

즉 제약을 걸면 **동명이인 두 명이 로그인하는 순간 두 번째 사람의 로그인이 실패한다.**
사용자가 고칠 방법도 없다. 자기 카카오 닉네임을 바꾸는 수밖에 없다.

## 선택지

### A. 유니크 제약 + 카카오 쪽도 충돌 회피

| 장점 | 단점 |
| --- | --- |
| 닉네임이 유일해짐 | 신규 가입과 재로그인 프로필 갱신 두 경로에 같은 회피 로직이 필요 |
| | 남이 먼저 썼다는 이유로 내 카카오 닉네임이 조용히 바뀐다 |

### B. 유니크를 걸지 않는다

| 장점 | 단점 |
| --- | --- |
| 카카오 로그인이 지금 그대로 돈다 | 같은 닉네임이 화면에 겹쳐 보일 수 있음 |
| 생성기는 뒤에 네 자리를 붙여 겹침을 줄이기만 하면 됨 | |

## 결정: **B. 유니크를 걸지 않는다**

비로그인 신원의 닉네임은 `형용사 + 명사 + 네 자리`로 만들되,
**그 네 자리는 유일성 보장이 아니라 겹쳐 보일 확률을 줄이는 장치**일 뿐이다.

## 근거

1. 사람을 가리키는 것은 `userId`지 닉네임이 아니다. 닉네임이 겹쳐도 서버 로직은 흔들리지 않는다
2. 로그인이 남의 닉네임 때문에 실패하는 것은 사용자가 손쓸 수 없는 실패다.
   겹쳐 보이는 불편보다 훨씬 나쁘다
3. 겹침이 실제로 문제가 되면 그때 **표시 시점에** 구분자를 붙이면 된다.
   저장 시점의 제약으로 풀 문제가 아니다
...
| 8. 스키마 | **Flyway.** 지금 구조를 `V1__init.sql`로. 테스트도 같은 스크립트 + `validate` 로 돌려 운영 DDL을 매 빌드 검증 |
| 9. 외래키 | **DB에 걸지 않는다.** 참조 무결성은 애플리케이션 책임. `@ForeignKey(NO_CONSTRAINT)` 명시 |
| 10. 타임존 | **저장은 UTC**, 표시는 클라이언트. JVM 기본값만 `Asia/Seoul`(로그 가독성) |
| 11. 토큰 전달 | 로그인 후 **프래그먼트**로 전달(`/auth/callback#token=`), `localStorage` 보관, STOMP 는 **CONNECT 프레임 헤더**. ADR-5 의 실행 방법 |
| 12. 비로그인 용어 | 코드는 **`VISITOR`**, 화면 문구는 "게스트". `guest` 는 용어집에서 이미 **방의 참가자**다 |
| 13. 발급 남용 | **인메모리 IP 리밋**(시간당 10회). 서버가 한 대인 동안. 판단은 `VisitorIssueLimiter` 한 곳에만 두어 Redis 로 갈아끼울 수 있게 함 |
| 14. 닉네임 유니크 | **걸지 않는다.** 걸면 동명이인의 카카오 로그인이 실패한다. 사람을 가리키는 것은 `userId` 다 |
