...
class MatchResultServiceTest extends IntegrationTestSupport {
    ...
    private static final long BLACK = 9001L;
    private static final long WHITE = 9002L;
    ...
    void setUp() {
        ...
        userRepository.saveAll(List.of(
                User.ofKakao(BLACK, "black@test.com", "흑", null), User.ofKakao(WHITE, "white@test.com", "백", null)));
    }
    ...
    class 등급_게임 {
        ...
        void 제로섬으로_반영된다() {
            ...
            int before = reload(BLACK).getMmr();

            // when
            matchResultService.record(BLACK, WHITE, BLACK, "RESIGN", RATED_MOVES);

            // then
            User black = reload(BLACK);
            User white = reload(WHITE);
        ...
        void 원장에_변동값이_남는다() {
            // given
            int blackBefore = reload(BLACK).getMmr();
            int whiteBefore = reload(WHITE).getMmr();
            ...
            assertThat(reload(BLACK).getMmr()).isEqualTo(blackBefore + match.getBlackMmrDelta());
        }
        ...
        void 최고_기록은_남는다() {
            ...
            matchResultService.record(BLACK, WHITE, BLACK, "RESIGN", RATED_MOVES);
            int peak = reload(BLACK).getPeakMmr();
            assertThat(peak).isEqualTo(reload(BLACK).getMmr());

            // when — 이번엔 진다
            matchResultService.record(BLACK, WHITE, WHITE, "RESIGN", RATED_MOVES);
        ...
        void 무승부가_반영된다() {
            // when
            matchResultService.record(BLACK, WHITE, null, "DRAW_AGREED", RATED_MOVES);

            // then
            assertThat(reload(BLACK).getDraws()).isEqualTo(1);
            assertThat(reload(WHITE).getDraws()).isEqualTo(1);
    ...
    class 어뷰징_대책 {
        ...
        void 조기_종료는_등급에_반영되지_않는다() {
            ...
            // when
            matchResultService.record(BLACK, WHITE, BLACK, "RESIGN", RATED_MOVES - 1);

            // then
            assertThat(reload(BLACK).getMmr()).isEqualTo(before);
            assertThat(reload(WHITE).getMmr()).isEqualTo(before);
            assertThat(reload(BLACK).getWins()).isEqualTo(1);
    ...
    class 배치_기간 {
        ...
        void 배치_경기는_변동이_크다() {
            ...
            for (int i = 0; i < Rating.PLACEMENT_GAMES; i++) {
                long opponent = 9100L + i;
                userRepository.save(User.ofKakao(opponent, opponent + "@test.com", "상대" + i, null));
                matchResultService.record(BLACK, opponent, BLACK, "RESIGN", RATED_MOVES);
            }
            long lastOpponent = 9200L;
            userRepository.save(User.ofKakao(lastOpponent, "last@test.com", "마지막", null));
            matchResultService.record(BLACK, lastOpponent, BLACK, "RESIGN", RATED_MOVES);

            // then
            List<Match> matches = matchRepository.findAll();
            int settledDelta = matches.get(matches.size() - 1).getBlackMmrDelta();
            assertThat(reload(BLACK).gamesPlayed()).isGreaterThan(Rating.PLACEMENT_GAMES);
